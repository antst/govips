...
func vipsJoin(input1 *C.VipsImage, input2 *C.VipsImage, dir Direction) (*C.VipsImage, error) {
	...
	// lifetime of the output. Unref'ing here dropped refs we never owned,
	// double-freeing the inputs once their ImageRefs were collected — an
	// intermittent use-after-free in libvips cache/dispose teardown (#1).
